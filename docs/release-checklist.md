...
검증 및 사용자 승인: 2026-09-22. [PR #1](https://github.com/HoSeopLee/lint/pull/1)을 병합하고 `1.0.0`과 `2.0.0`을 배포했습니다. `2.0.0`은 영문·한국어 README와 `broccoil-react` 규칙 이름을 정리한 뒤 사용자 승인을 받아 게시했습니다.
...
게시된 `2.0.0`의 gitHead는 `ec6a3b4113a148f285bc2a5c31fb943b359c9e7b`, shasum은 `26cbcef6e3701c06112bd53d5091000fb06adc13`입니다. `v2.0.0` Git 태그는 해당 소스 커밋을 가리킵니다. 최종 npm 태그는 `legacy → 1.0.0`, `latest → 2.0.0`이며, npm에 표시되는 README가 영문 README와 일치하는지 확인했습니다. 이미 게시한 `1.0.0`은 재배포하지 않았습니다.
...
| README의 pnpm 설치·React/Next 검사 (pnpm 11.13.0 / Node 24.18.0) | 게시된 1.0.0 통과 | 게시된 2.0.0 통과 |

영문 `README.md`와 한국어 `README.ko.md`를 함께 제공합니다. 두 문서의 실행 예제가 같은지 확인했고, 한국어 문서도 배포 파일 목록에 추가했습니다. 이미 게시된 1.0.0은 기존 19개 파일을 유지합니다.

실제 게시된 `2.0.0`을 pnpm으로 설치한 React/Next 소비 프로젝트에서 공개 경로 로드, 정상 코드·오류 진단, import 정렬·미사용 import 제거, Next 규칙과 `broccoil-react/no-unsafe-target-blank` 규칙 ID를 확인했습니다.
...
릴리스 소스의 GitHub CI 결과: [1.x · a9e411b](https://github.com/HoSeopLee/lint/actions/runs/35700288193), [main · ec6a3b4](https://github.com/HoSeopLee/lint/actions/runs/35702443865). 두 계열 모두 최소 Node와 Node 24 조합을 통과했습니다.
...
- [x] README 수정본 확인 후 `2.0.0` npm 배포
- [x] `2.0.0` 실제 설치 검사·Git 태그 기록 및 최종 dist-tag 확인
